...
<head>
  ...
  {{-- Favicon --}}
  <link rel="icon" type="image/jpeg" href="{{ asset('image/logo_bcake.jpg') }}">
  <link rel="apple-touch-icon" href="{{ asset('image/logo_bcake.jpg') }}">
  ...
  <style>
    :root{ --bcake-wine:#890524; --bcake-deep:#57091d; }
...
<body class="bg-rose-50 min-h-screen">
  ...
  {{-- ===== TOP BAR ===== --}}
  <header class="bg-white/80 backdrop-blur border-b border-rose-200/60 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 lg:px-8 h-14 flex items-center justify-between">
      ...
      <a href="{{ Route::has('home') ? route('home') : '#' }}" class="inline-flex items-center gap-3">
        <img src="{{ asset('image/logo_bcake.jpg') }}" class="h-8 w-8 rounded-lg object-cover ring-1 ring-rose-200/60" alt="B’cake">
        <span class="font-display text-xl text-rose-800">B’cake Dashboard</span>
      </a>
      ...
      <div class="hidden lg:flex items-center gap-2">
        ...
        @else
          @if (Route::has('login'))
            <a href="{{ route('login') }}" class="rounded-full border px-4 py-1.5 text-sm hover:bg-rose-50">Login</a>
          @endif
          @if (Route::has('register'))
            <a href="{{ route('register') }}" class="rounded-full bg-rose-700 text-white px-4 py-1.5 text-sm hover:bg-rose-800">Daftar</a>
          @endif
        @endauth
      </div>
    </div>
  ...
  <div class="max-w-7xl mx-auto px-4 lg:px-8 py-6 grid grid-cols-12 gap-6">
    ...
    {{-- ===== SIDEBAR ===== --}}
    <div id="sbBackdrop" class="fixed inset-0 bg-black/30 z-40 hidden lg:!hidden"></div>
    ...
    <aside id="sidebar"
      ...
      <div class="flex items-center gap-3 mb-6">
        <img src="{{ asset('image/logo_bcake.jpg') }}" class="h-10 w-10 rounded-lg object-cover ring-1 ring-rose-200/60" alt="Logo" loading="lazy">
        <div>
      ...
      <nav class="space-y-1 text-[15px]">
        <a href="{{ Route::has('seller.dashboard') ? route('seller.dashboard') : '#' }}"
           class="nav-item {{ request()->routeIs('seller.dashboard') ? 'nav-active' : 'text-gray-700' }}">🏠 Dashboard</a>

        <a href="{{ Route::has('seller.products.index') ? route('seller.products.index') : '#' }}"
           class="nav-item {{ request()->routeIs('seller.products.*') ? 'nav-active' : 'text-gray-700' }}">🍰 Produk</a>

        <a href="{{ Route::has('cart.index') ? route('cart.index') : '#' }}"
           class="nav-item {{ request()->is('cart*') ? 'nav-active' : 'text-gray-700' }}">🛒 Keranjang</a>

        <a href="#" class="nav-item text-gray-700">📣 Promo</a>
        <a href="#" class="nav-item text-gray-700">⚙️ Pengaturan</a>
  ...
  {{-- ===== FOOTER ===== --}}
  <footer class="border-t border-rose-200/50 bg-white mt-8">
    <div class="max-w-7xl mx-auto px-4 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between text-sm text-gray-500 gap-2">
